...
test.describe('06. Profile Editing Tests', () => {
  ...
  test(
    'TC-02: should update social address',
    { tag: ['@mutation'] },
    async ({ page, context, layoutPage, loginPage }) => {
      await addCookies(context, 'kunsthalte')
      await layoutPage.gotoHome('kunsthalte')
  ...
  test(
    'TC-04: should update password and display new credentials',
    { tag: ['@mutation'] },
    async ({ page, context, layoutPage, loginPage, profilePage }) => {
  ...
  test('TC-05: should not update password for invalid structure', async ({
    page,
    context,
    layoutPage,
    loginPage,
